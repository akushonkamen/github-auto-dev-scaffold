...
echo "     - LLM_API_KEY     = your Zhipu GLM API key"
...
echo "     - TRIAGE_MODEL           = glm-5.2"
echo "     - DEVELOP_MODEL          = glm-5.2"
...
echo ""
echo "     d) Single-module test — Self-verify:"
echo "        After develop creates a PR for a claude/issue-N-* branch,"
echo "        self-verify fires automatically on pull_request.opened."
echo "        Expected: 'verifying' label → 'verified' (or 'verify:failed')"
echo "        + audit comment on the issue with verify report."
echo ""
echo "     e) Full pipeline:"
echo "        Open a well-specified bug report."
echo "        Expected: triage → (high conf) → accepted → develop → PR → self-verify → verified."
echo ""
echo "     f) Chinese issue:"
echo "        Open an issue in Chinese."
echo "        Expected: claude responds in Chinese at every stage."
echo ""
echo "     g) Prompt injection fixture:"
echo "        Submit the content of .github/fixtures/hostile-injection.md as an issue."
echo "        Expected: yielded (not accepted-by-claude)."
echo ""
echo "     h) Module 5 e2e:"
echo "        Open an issue with clear acceptance criteria, let develop implement,"
echo "        self-verify fires on the resulting PR, verify report appears on issue."
echo ""
echo "     i) Module 6 e2e:"
echo "        After Module 5 self-verify passes and applies 'verified' label,"
echo "        test workflow fires automatically. Expected: 'testing' label →"
echo "        Codex runs tests → 'tested' (or 'test:failed') label + test report"
echo "        audit comment on the PR. Requires OPENAI_API_KEY secret and"
echo "        Codex CLI installed on runner (or fallback to no-op)."
echo ""
